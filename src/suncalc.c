...
float my_suntimes(float lat, float lon, PblTm time, float timezone, double angle) {
	times return_times;
	...
	PblTm solar_noon, sunrise, sunset;
	...
	return_times.sunip = excel_to_pebble_time(sunrise_time_LST);
...
PblTm excel_to_pebble_time(double excel_time)
{
	...
	time.tm_hour = excel_time%(1.0/24.0);	//Hours since midnight.
	excel_time -= time.tm_hour*(1.0/24.0);	//Remove hour portion
	time.tm_min = excel_time%(1.0/24.0/60.0);	//Minutes after the hour.
	excel_time -= time.tm_min*(1.0/24.0/60.0);	//Remove minute portion
	time.tm_sec = excel_time%(1.0/24.0/60.0/60.0);	//Seconds after the minute.
