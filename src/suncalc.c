...
float my_sunrise(float lat, float lon, PblTm time, float timezone) {
    ...
    double julian_day, julian_century, geom_mean_long_sun_deg, geom_mean_anom_sun_deg, eccent_earth_orbit, sun_eq_of_ctr, sun_true_long_deg, sun_app_long_deg;
...
V	Eq of Time (minutes)	-1.706307841	=4*DEGREES(U2*SIN(2*RADIANS(geom_mean_long_sun_deg))-2*eccent_earth_orbit*SIN(RADIANS(sun_eq_of_ctr))+4*eccent_earth_orbit*U2*SIN(RADIANS(sun_eq_of_ctr))*COS(2*RADIANS(geom_mean_long_sun_deg))-0.5*U2*U2*SIN(4*RADIANS(geom_mean_long_sun_deg))-1.25*eccent_earth_orbit*eccent_earth_orbit*SIN(2*RADIANS(sun_eq_of_ctr)))
...
float my_sunset(float lat, float lon, float timezone) {
	Date	Time (past local midnight)	Julian Day	julian_century		Geom Mean Long Sun (deg)	Geom Mean Anom Sun (deg)	Eccent Earth Orbit	Sun Eq of Ctr	Sun True Long (deg)	Sun True Anom (deg)	Sun Rad Vector (AUs)	Sun App Long (deg)	Mean Obliq Ecliptic (deg)	Obliq Corr (deg)	Sun Rt Ascen (deg)	Sun Declin (deg)	var y	Eq of Time (minutes)	HA Sunrise (deg)	Solar Noon (LST)	Sunrise Time (LST)	Sunset Time (LST)
